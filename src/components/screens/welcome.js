...
const Welcome = (props) => {
  ...
  useEffect(() => {
    ...
    handleTimer();
    //unmount, giving problem after react18 upgrade
  ...
  return (
    <div className="center parent-center">
      <Modalify tagid="modalwelcome" show={isWelcome} close={onClose}>
        <div className="child-center">
          ...
          <div>
            ...
            <span className="xred">
              its not saved anywhere but your session only
            </span>
            <input
              type="text"
              placeholder="what's you name, skip if you dont want it"
              onChange={(e) => setName(e.target.value)}
            />
            <button className="btn purple xwhite" onClick={() => onClose()}>
              proceed now
            </button>
          </div>
        </div>
